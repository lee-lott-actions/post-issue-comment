Describe "Add-IssueComment" {
    ...
    It "fails with HTTP 403" {
        ...
        $output | Should -Contain "error-message=Failed to post comment to issue #1.  Status: 403"
    }
    ...
    It "fails with HTTP 404" {
        ...
        $output = Get-Content $env:GITHUB_OUTPUT
        $output | Should -Contain "result=failure"
        $output | Should -Contain "error-message=Failed to post comment to issue #1.  Status: 404"
    }
